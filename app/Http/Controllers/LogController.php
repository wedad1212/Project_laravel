...
class LogController extends Controller
{
    public function index()
	{
		...
		for($i=0; $i<count($logs); $i++){
            ...
            $issue = DB::table('book_issues')->where('book_id', $issue_id)->first();
	...
	public function edit($id)
	{
		...
		if(!$log->count()){
			...
			DB::transaction( function() use($log_id, $student_id, $issue_id) {
				...
				$student = Student::find($student_id);
				$student->books_issued = 0;
				$student->save();

				// change issue availability status
                DB::table('book_issues')->where(['book_id' => $issue_id])->update(array('available_status' => 1));
			});
